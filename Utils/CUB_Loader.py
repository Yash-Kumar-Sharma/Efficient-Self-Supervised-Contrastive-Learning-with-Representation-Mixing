...
class MixingCUB(VisionDataset):
    ...
    def __init__(self, root, K,train=True, transform=None, target_transform=None, download=False):
        super(MixingCUB, self).__init__(root)
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None and self.target_transform is not None:
            for _ in range(self.K):
                ...
                sample2 = self.data.iloc[randNumber]
                path = os.path.join(self.root, self.base_folder, sample2.filepath)
                #target = sample.target - 1  # Targets start at 1 by default, so shift to 0
                pic2 = self.loader(path)

                img_transformed = self.transform(pic2.copy())
                img_list.append(img_transformed)

                img_transformed = self.target_transform(pic2.copy())
                img_trans_list.append(img_transformed)

                img_transformed = self.target_transform(pic.copy())
                img_trans_list.append(img_transformed)
        else:
...
class MultiCUB(VisionDataset):
    ...
    def __init__(self, root, K, train=True, transform=None, target_transform=None, download=False):
        super(MultiCUB, self).__init__(root)
...
class CUB_Data(VisionDataset):
    ...
    def __init__(self, root, train, transform=None, target_transform=None, download=False):
        super(CUB_Data, self).__init__(root)
