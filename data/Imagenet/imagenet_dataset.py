...
class Imagenet_DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        #train_data = torchvision.datasets.ImageFolder(os.path.join(config.data_dir,"train"))
        #resize = torchvision.transforms.Resize(size=(config.image_size, config.image_size))
        #normalize = torchvision.transforms.Normalize(mean = config.mean, std = config.std)
        
        if(self.model_name == "Our"):
            print("Our Data")
            self.train_set = OurDataFromImagenet(K=1, path = self.data_dir,
                                     root = self.data_dir,
        ...
        if(self.model_name == "Simclr" or self.model_name =="SimclrV2" or
            ...
            print("Simclr Data")
            self.train_set = SimclrDataFromImagenet(K=self.K, path = self.data_dir,
                                     root = self.data_dir,
                                     transform = self.preprocess,
                                     split = 'train',
                                     #train = True,
        ...
        if(self.model_name == "Moco"):
            print("Moco Data")
            self.train_set = MocoDataFromImagenet(root = self.data_dir, path = self.data_dir,
                                     transform = self.preprocess,
                                     split = 'train',
                                     #train = True,
                                     )
