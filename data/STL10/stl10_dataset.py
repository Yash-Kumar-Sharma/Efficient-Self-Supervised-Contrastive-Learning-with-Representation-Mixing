...
from data.OurModel_Data import OurDataFromSTL10
from data.Simclr_Data import SimclrDataFromSTL10
from data.Moco_Data import MocoDataFromSTL10
...
class Stl10_DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if(self.model_name == "Our"):
            print("Our Data")
            self.train_set = OurDataFromSTL10(K=self.K,
                                     root = self.data_dir,
                                     path = self.data_dir,
        ...
        if(self.model_name == "Simclr" or self.model_name =="SimclrV2" or
            ...
            print("Simclr Data")
            self.train_set = SimclrDataFromSTL10(K=self.K,
                                     root = self.data_dir,
                                     path = self.data_dir,
                                     transform = self.preprocess,
                                     split = 'unlabeled',
        ...
        if(self.model_name == "Moco"):
            print("Moco Data")
            self.train_set = MocoDataFromSTL10(root = self.data_dir,
                                     path = self.data_dir,
                                     transform = self.preprocess,
                                     #train = True,
                                     split = 'unlabeled',
