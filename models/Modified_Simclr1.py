...
class Modified_Simclr1Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx): 
        ...
        train_x, train_x_data = batch
        self.optimizer.zero_grad()
        embeddings = self.net(train_x)
        self.norm_embeddings = self.net(train_x_data)
        
        self.embeddings = embeddings.reshape(self.K,int(self.batch_size),int(self.projection_size))
        
        loss = 0
        for _ in range(self.embeddings.shape[0]):
            loss += xent_loss(self.embeddings[0], self.norm_embeddings)
        
        self.loss_metric.update(loss)
        pos_mean = 0
        neg_mean = 0
        for _ in range(self.embeddings.shape[0]):
            pos, neg = Modified_Positive_Negative_Mean(x = self.embeddings[0], y = self.norm_embeddings, device = self.global_rank)
            pos_mean += pos
            neg_mean += neg
        self.pos_mean += pos_mean * len(train_x)
        self.neg_mean += neg_mean * len(train_x_data)
        self.total += len(train_x) + len(train_x_data)
